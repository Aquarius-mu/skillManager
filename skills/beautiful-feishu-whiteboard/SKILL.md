...
label: 更美观的画板
version: 1.1.1-game.2
description: 生成美观、可编辑的飞书画板（Whiteboard），偏向技术/代码实现方向——代码架构、模块/类关系、调用链、协议消息结构、数据结构与存储、状态机、算法控制流等。内置 35 种精选配色风格，输入标题、主题即可自动匹配布局与配色，写入飞书云文档，支持风格切换。触发关键词：画板、whiteboard、飞书画板、架构图、类图、时序图、流程图、状态机、数据结构、协议图、思维导图。
---
